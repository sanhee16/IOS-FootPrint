...
class Util {
    ...
    static func makeFootprintCopy(_ footprint: FootPrint) -> FootPrint {
        return FootPrint(title: footprint.title, content: footprint.content, images: footprint.images, latitude: footprint.latitude, longitude: footprint.longitude, tag: footprint.tag, peopleWithIds: footprint.peopleWithIds, placeId: footprint.placeId, address: footprint.address)
    }
    ...
    static func getSettingStatus(_ flag: SettingFlag) -> Bool {
        let settingFlag: Int = Defaults.SettingFlag
        let idx: Int = flag.rawValue
        let res: Int = settingFlag & (0b1 << idx)
        return res > 0
    }
    ...
    static func setSettingStatus(_ flag: SettingFlag, isOn: Bool) {
        ...
        let settingFlag: Int = Defaults.SettingFlag
        let idx: Int = flag.rawValue
        let flag: Int = isOn ? 0b1 : 0b0
        let calc: Int = 0b11111111 & (flag << idx)
        let res: Int = settingFlag | calc
        print("settingFlag: \(settingFlag)")
        print("idx: \(idx)")
        print("flag: \(flag)")
        print("calc: \(calc)")
        print("res: \(res)")
